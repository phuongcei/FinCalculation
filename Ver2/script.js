...
function calculateTrading() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        const dailyProfit = currentAccounts * capitalPerAccount * dailyRate;
        cashPool += dailyProfit;
        ...
        // Record Data points (every 5 days or if new account added to avoid too many points)
        if (day % 5 === 0 || newAccountsToday > 0 || day === totalDays) {
            labels.push(`Day ${day}`);
            dataAccounts.push(currentAccounts);
            const currentNetWorth = (currentAccounts * capitalPerAccount) + cashPool;
            dataNetWorth.push(currentNetWorth);

            // Check X2 Milestone
            if (timeToX2 === null && currentNetWorth >= initialTotalCapital * 2) {
                timeToX2 = day;
                logHTML += `<div style="color: #10b981; margin-bottom: 5px;">Day ${day}: 🚀 Đạt X2 Vốn! (Net Worth: ${formatCurrency(currentNetWorth)})</div>`;
            }
